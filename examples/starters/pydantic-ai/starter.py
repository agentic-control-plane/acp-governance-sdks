...
Governance pattern: hooks (Pydantic AI >= 2). `ACPHooks()` registers a
`Hooks` capability on the agent whose `tool_execute` wrap hook governs
EVERY tool — no per-function decorators. Denials mean the tool never
runs; the model receives "tool_error: <reason>" as the tool result and
adapts. Redacted outputs replace the original before the model sees them.
...
configure(
    ...
    client_header="acp-pydantic-ai-starter/0.2.0",
)

# ── 3. Build the agent, with ACP registered as a capability. This one
# line governs every tool on the agent — the one below and any you add.
...
# ── 4. Your tool. Just `@agent.tool_plain` — ACPHooks already covers it.
